...
全局锁就是对整个数据库实例进行加锁。`MySQL`中进行加**全局读锁**的命令是`FLUSH TABLES WITH READ LOCK (FTWRL)`。

当想要整个库处于只读状态的时候，可以使用这个命令，之后其他线程的以下的语句会被阻塞：数据更新语句、数据定于语句、更新类事务的提交语句。

全局锁的典型的使用场景是做全库的逻辑备份。让整个库处于只读状态，会有这些情况出现：

- * 如果想要在主库上备份，那么在备份期间不能执行更新，业务基本处于停摆；

- * 如果想要在从库上备份，那么在备份期间从库不能执行主库同步过来的`binlog`，会导致主从延迟。

- * 而如果备份时候不进行加锁的话，会导致备份得到的库不是一个逻辑时间点，即最终的视图是逻辑不一致的。

为了解决这个问题，可以使用官方的逻辑备份工具`mysqldump`。当其使用参数`--single-transaction`的时候，导出数据之前会启动一个事务，来确保一致性视图。而由于`MVCC`的支持，这个过程中的数据是可以正常更新的。

但是某些数据库引擎并不支持`MVCC`，这时候就需要使用`FTWRL`命令了。

***注意：*** 并不建议使用`SET GLOBAL READONLY=true`的方式来让全库只读，因为`GLOBAL`的影响面更大，同时当客户端出现异常断开的时候，数据库就会一直保持在`READONLY`状态，而`FTWRL`命令会在出现异常的时候，自动释放这个全局锁。

* 表级锁

`MySQL`中的表级锁有两种：表锁和元数据锁(`meta data lock, MDL`)。
...
`MDL` 是在 `MySQL 5.5` 中引入的，当对一个表做增删改查操作的时候，加 `MDL` 读锁；当要对表做结构变更操作的时候，加 `MDL` 写锁。

`MDL` 不需要显式使用，在访问一个表的时候会被自动加上。其作用是保证读写的正确性。

读锁之间不互斥，因此你可以有多个线程同时对一张表增删改查。

读写锁之间、写锁之间是互斥的，用来保证变更表结构操作的安全性。因此，如果有两个线程要同时给一个表加字段，其中一个要等另一个执行完才能开始执行。

事务中的 `MDL` 锁，是在语句执行开始时申请，但是语句结束后并不会马上释放，而会等到整个事务提交后再释放。

考虑这样一个场景：

当一个修改表结构的事务启动后，所有的读请求是被阻塞的，而如果客户端存在重试的机制，这个库的线程很快就会爆满。

这时候就需要减少长事务的出现。因为如果事务不提交，就会一直占用这个 `MDL` 锁。

* 行锁

行锁是在引擎的基础上实现的，`MySQL`中`MyISAM`是不支持行锁的，而`InnoDB`是支持行锁的。

行锁就是针对数据表中行记录的锁。这很好理解，比如事务 `A` 更新了一行，而这时候事务 `B` 也要更新同一行，则必须等事务 `A` 的操作完成后才能进行更新。

在 `InnoDB` 事务中，行锁是在需要的时候才加上的，但并不是不需要了就立刻释放，而是要等到**事务结束时才释放**。这个也是**两阶段锁协议**。

当事务中需要锁多个行的时候，要把最可能的造成锁冲突、最可能影响并发度的锁尽量往后放。

* 死锁

当并发系统中不同线程出现循环资源依赖，涉及的线程都在等待别的线程释放资源时，就会导致这几个线程都进入无限等待的**死锁**状态。

在多个事务并发执行，出现死锁时候，常用两种策略来解决死锁：

1) 设置一个超时时间，当死锁发生时，直接进入等待，超时后退出。可以通过参数`innodb_lock_wait_timeout`来进行设置；

2) 发起死锁检测，发现死锁后，主动回滚死锁链条中的某一个事务，让其他事务得以继续执行。将参数 `innodb_deadlock_detect` 设置为 `on`，表示开启这个逻辑。

第`1`种方式，设置的等待时间很重要，如果超时时间过长，对于某些敏感或者在线服务，会比较难以接受。

而第`2`种方式，因为每进行一次死锁检测，都会消耗一定的`CPU`资源，当操作数较多的时候，就会导致大量`CPU`资源的消耗。这时候，需要考虑的就是怎么控制并发的操作数。
