...
$$\alpha_{1}^{n e w} y_{1}+\alpha_{2}^{n e w} y_{2}=\alpha_{1}^{o l d} y_{1}+\alpha_{2}^{o l d} y_{2}=\zeta$$

其中$\zeta$为常数。

求解的方法是先求解$\alpha_{2}^{n e w}$，然后利用其表示$\alpha_{1}^{n e w}$。

为了求解$\alpha_{2}^{n e w}$，假设它的上下界分别为$H, L$，就有：

$$L \leq \alpha_{2}^{n e w} \leq H$$

下面就综合$0 \leq \alpha_i \leq C, i = 1, \cdots, n$和$ \alpha_{1}^{n e w} y_{1}+\alpha_{2}^{n e w} y_{2}=\alpha_{1}^{o l d} y_{1}+\alpha_{2}^{o l d} y_{2}=\zeta$来求$\alpha_{2}^{n e w}$的取值范围。

1). $y_1 \neq y_2$

即两个样本分别属于不同类别时，有：$\alpha_{1}^{o l d}-\alpha_{2}^{o l d}=\zeta$
...
即两个样本分别属于同一类别时，有：$\alpha_{1}^{o l d}+\alpha_{2}^{o l d}=\zeta$

所以有：

$$L=\max (0, \zeta-C), H=\min (C, \zeta)$$

综上，可以得到$\alpha_{2}^{n e w}$的上下界为：
