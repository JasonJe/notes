...
$x\in \Bbb{R}^n$是输入，$Y\in {0, 1}$是输出，$\omega \in \Bbb{R}^n$为权值向量，$b \in \Bbb{R}$为偏置，$\omega \cdot x$为$\omega$和$x$的内积。
...
而$h_\theta(x)$只与$\theta^Tx$有关，当$\theta^Tx >> 0$时，$h_\theta(x) = 1$，反之$h_\theta(x) = 0$。

$g(z)$是一个映射，其将$z = \omega^T + b$，最终形成的简单映射关系如下：

$$g(z) =\begin{cases}  1, & z\geq 0 \\ -1, & z < 0 \end{cases}$$

#### 函数间隔和几何间隔

根据上文提出的超平面，其可以用$f(x) = \omega^Tx + b$表示，如果假设数据空间是一个二维平面，超平面就是下图所示的一条直线，当$f(x) = 0$时，$x$就位于超平面上，$f(x) > 0$对应$y = 1$的数据点，$f(x) < 0$对应$y = -1$的数据点。

![超平面](../assets/images/824_01.png)

* 函数间隔

在超平面确定的情况下，$\mid \omega^Tx + b \mid$能够表示点$x$距离超平面的远近，通过观察$\omega^Tx + b$的符号和类标记$y$的符号可以判断出分类是否正确，即通过$y \times (\omega^Tx + b)$的正负性来判定分类的正确性，由此引出函数间隔(`Functional Margin`)：

$$\hat \gamma = y(\omega^Tx + b) = y f(x)$$

超平面关于样本集的函数间隔最小值就是关于训练集的函数间隔：

$$\hat \gamma = \mathbf{min} \gamma $$

当成比变化$\omega^T, b$时候，函数间隔也会成比例变化，所以只有函数间隔远远不够。

* 几何间隔

可以对法向量$\omega$增加约束来引出真正定义的点到超平面的距离，几何间隔(`Geometrical Margin`)。

假设一个点$x$垂直投影到超平面上对应的点为$x_0$，$\omega$是垂直于超平面的一个向量，$\gamma$为样本$x$到超平面的距离，如下图：

![几何间隔](../assets/images/824_02.png)

$$x = x_0 + \gamma \frac{\omega}{||\omega||}$$

其中$\frac{\omega}{||\omega||}$是单位向量。

根据$\omega^Tx_0 = -b, \omega^T\omega = ||\omega||^2$可以得到

$$\gamma = \frac{\omega^Tx + b}{||\omega||} + \frac{f(x)}{||\omega||}$$

为了得到$\gamma$的绝对值，令$\gamma$乘上对应的类别$y$，即可得出几何间隔$\overline \gamma$：

$$\overline \gamma = y \gamma = \frac{\overline \gamma}{||\omega||}$$

几何间隔就是函数间隔除以$||\omega||$，其才是直观上的点到超平面的距离。
...
$$y_i(\omega^Tx_i + b) = \overline \gamma_i \geq \overline \gamma, i = 1, ..., n$$

根据几何间隔的定义$\overline \gamma = y \gamma = \frac{\overline \gamma}{||\omega||}$，假设$\overline \gamma = 1$，则有$\overline \gamma = \frac{1}{||\omega||}$，$y_i(\omega^Tx_i + b) \geq 1, i = 1, 2, ..., n$，目标函数转化为：

$$\mathbf{max} \frac{1}{||\omega||}, s.t., y_i(\omega^Tx_i + b)\geq 1, i = 1, 2, ..., n$$

上式的意思就是在约束条件$y_i(\omega^Tx_i + b)\geq 1, i = 1, 2, ..., n$下，最大化$\mathbf{max} \frac{1}{||\omega||}$，其中$\frac{1}{||\omega||}$就是几何间隔$\overline \gamma$。

![支持向量](../assets/images/824_03.png)


如上图所示，中间实线就是最优超平面，两根虚线到实现的距离都为$\overline \gamma$，虚线间隔上的点就是支持向量，满足$y(\omega^Tx + b) = 1$。
...
而求$\frac{1}{||\omega||}$的最大值相当于求$\frac{1}{2} ||\omega||^2$的最小值，上述目标函数等价于：

$$\mathbf{min} \frac{1}{2} ||\omega||^2, s.t., y_i(\omega^Tx_i + b)\geq 1, i = 1, 2, ..., n$$
...
$$\cal{L} (\omega, b, \alpha) = \frac{1}{2} ||\omega||^2 - \sum_{i = 1}^{n}\alpha_i(y_i(\omega^Tx_i + b) - 1)$$

令$\theta(\omega) = \mathbf{max}_{\alpha_i \geq 0} \cal{L} (\omega, b, \alpha)$

当样本点不满足约束条件时，即在可行解区域外：$y_i(\omega^T x_i +b) < 1$，此时将$\alpha_i$设为$+\infty$，此时$\theta(\omega)$也是$+ \infty$的。

当样本点满足约束条件时，即在可行解区域内：$y_i(\omega^Tx_i + b) \geq 1$，此时，$\theta(\omega)$就是原目标函数本身。

所以综上得到新目标函数：

$$ \theta(\omega) = \begin{cases} \frac{1}{2} ||\omega||^2, & y_i(\omega^Tx_i + b) - 1 \geq 0 \\ +\infty, & y_i(\omega^T x_i +b) - 1 < 0 \end{cases} ​$$

由上，就建立起了在可行解区域内与原目标函数相同，在可行解区域外函数值趋近于$+\infty$的新函数，即：

$$\mathbf{min}_{\omega,b} \theta(\omega) = \mathbf{min}_{\omega, b }\mathbf{max}_{\alpha_i \geq 0} \cal{L}(\omega, b, \alpha) = p^*$$

这里的$p^*$与原问题是等价的，直接针对$\omega, b$和$\alpha_i$这个不等式约束进行求解，不易。这里再做一次变换：

$$\mathbf{max}_{\alpha_i \geq 0}\mathbf{min}_{\omega, b} \cal{L}(\omega, b, \alpha) = d^*$$

变换后的新问题是未做变换的对偶问题，其最优值用$d^*$表示，其中$d^* \leq p^*$，在满足某些条件的情况下，两者相等，可以通过求解对偶问题来简介求解原始问题。

求解过程就是先求$\cal {L}$对$\omega, b$的极小，再求$\cal {L}$对$\alpha$的极大。
...
$$ \cal{L}(\omega, b, \alpha) = f(x) + \sum_{i} \alpha_ig_i(x) + \sum_j b_j h_j(x) ​$$

`Slater`条件的含义就是：存在一个$x$，使得不等式约束$g_i(x) \leq 0$严格成立，这是原问题等价于对偶问题的一个充分条件，确保了鞍点的存在。

在上式中，即需要约束条件$y_i(\omega^T x_i +b) - 1 \geq 0$。
