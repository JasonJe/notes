...
$$\triangledown_{w}L(w) =\sum_{i=1}^{N}(y_i - \pi(x_i)) x_i$$
...
##### [完整的决策树实现(分类和回归)](../assets/scripts/decision_tree.py)
...
$$\mathcal{L}(\boldsymbol{w}, \boldsymbol{\alpha}, \boldsymbol{\beta})=\frac{1}{2}\|\boldsymbol{w}\|^{2}-\sum_{i=1}^{n} \alpha_{i}\left(y_{i}\left(\boldsymbol{w}^{\mathrm{T}} \boldsymbol{x}_{i}+b\right)-1\right)$$

令$\theta(\boldsymbol{w}) = \mathbf{max}_{\alpha_i \geq 0} \mathcal{L}(\boldsymbol{w}, \boldsymbol{\alpha}, \boldsymbol{\beta})$
...
$$\mathbf{min}_{w,b} \theta(\boldsymbol{w}) = \mathbf{min}_{w, b }\mathbf{max}_{\alpha_i \geq 0} \mathcal{L}(\boldsymbol{w}, \boldsymbol{\alpha}, \boldsymbol{\beta}) = p^*​$$

这里的$p^*$与原问题是等价的，直接针对$w, b$和$\alpha_i$这个不等式约束进行求解，不易。这里再做一次变换：

$$\mathbf{max}_{\alpha_i \geq 0}\mathbf{min}_{w, b} \mathcal{L}(\boldsymbol{w}, \boldsymbol{\alpha}, \boldsymbol{\beta}) = d^*$$
...
$$ \mathcal{L}(\boldsymbol{w}, \boldsymbol{\alpha}, \boldsymbol{\beta}) = f(x) + \sum_{i} \alpha_ig_i(x) + \sum_j \beta_j h_j(x) ​$$
...
$$\begin{array}{ll} \mathbf{min} & f(x) \\ {\text {s.t.}} & {h_j(x) = 0, j = 1, 2, ..., p} \\ {} & {g_k(x) \leq 0, k = 1, 2, ..., q} \\ {} & {x \in \mathbf{X} \subset \Bbb{R}^n} \end{array}$$
...
1. 针对上面两个式子，首先求解$\mathbf{min}_{w, b} \mathcal{L}(\boldsymbol{w}, \boldsymbol{\alpha}, \boldsymbol{\beta})​$。

固定$\boldsymbol{\alpha}​$，让$\mathcal{L}(\boldsymbol{w}, \boldsymbol{\alpha}, \boldsymbol{\beta})​$关于$\boldsymbol{w}, b​$最小化，分别对$\boldsymbol{w}, b​$求偏导，令偏导等于$0​$，有：
...
$$\begin{array}{ll}\mathbf{max}_{\boldsymbol{\alpha}} & {\sum_{i=1}^{n} \alpha_{i}-\frac{1}{2} \sum_{i, j=1}^{n} \alpha_{i} \alpha_{j} y_{i} y_{j} \boldsymbol{x}_{i}^{\mathrm{T}} \boldsymbol{x}_{j}} \\
{\text { s.t. }} & {\alpha_{i} \geq 0, i=1,2, \cdots, n} \\
{} & {\sum_{i=1}^{n} \alpha_{i} y_{i}=0} \end{array}​$$

进行变形，即将最大值问题转变为最小值问题，有：

$$\begin{array}{ll} \mathbf{min}_{\boldsymbol{\alpha}} &{\frac{1}{2} \sum_{i=1}^{n} \sum_{j=1}^{n} \alpha_{i} \alpha_{j} y_{i} y_{j} \boldsymbol{x}_{i}^{T} \boldsymbol{x}_{j}-\sum_{i=1}^{n} \alpha_{i}} \\ {\text { s.t. }} & {\alpha_{i} \geq 0, \quad i=1,2, \cdots, n} \\ {}& {\sum_{i=1}^{n} \alpha_{i} y_{i}=0} \end{array}​$$

上述目标函数是在假设样本$100\%$线性可分得出的，所以在实际情况中，需要引入松弛变量$\xi$和惩罚参数$C$。

这时候约束条件变成：

$$\begin{array}{ll} {\text { s.t. }} & {C \geq \alpha_{i} \geq 0, \quad i=1,2, \cdots, n} \\ {} & {\sum_{i=1}^{n} \alpha_{i} y_{i}=0​}\end{array} $$
...
$$\mathcal{L}(\boldsymbol{w}, \boldsymbol{\alpha}, \boldsymbol{\beta}, \boldsymbol{\xi})=\frac{1}{2}\|w\|^{2}+C \sum_{i=1}^{N} \xi_{i}-\sum_{i=1}^{N} \alpha_{i}\left[y_{i}\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right)-1+\xi_{i}\right]-\sum_{i=1}^{N} \left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right) \xi_{i}$$

先让$\mathcal{L}$对$\boldsymbol{w}, \boldsymbol{\beta}, \boldsymbol{\xi}$最小化：

$$\begin{aligned} \frac{\partial \mathcal{L}}{\partial w} &=0 \Rightarrow w=\sum_{i=1}^{n} \alpha_{i} y_{i} x_{i} \\ \frac{\partial \mathcal{L}}{\partial \beta} &=0 \Rightarrow \sum_{i=1}^{n} \alpha_{i} y_{i}=0 \\ \frac{\partial \mathcal{L}}{\partial \xi_{i}} &=0 \Rightarrow C-\alpha_{i}-\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right)=0, \quad i=1, \ldots, n \end{aligned}​$$

化简，优化问题为：

$$\begin{array}{ll}{\mathbf{min}_{\boldsymbol{\alpha}}} & {\frac{1}{2} \sum_{i=1}^{N} \sum_{j=1}^{N} \alpha_{i} \alpha_{j} y_{i} y_{j}\left(x_{i} \cdot x_{j}\right)-\sum_{i=1}^{N} \alpha_{i}} \\ {\text { s.t. }} & {\sum_{i=1}^{N} \alpha_{i} y_{i}=0} \\ {} & {0 \leq \alpha_{i} \leq C, \quad i=1,2, \cdots, n}\end{array}​$$

需要满足的`KKT`条件的对偶互补条件为：

$$\begin{align} \alpha_{i}\left(y_{i}\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right)-1+\xi_{i}\right)=0 \\ \left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right) \xi_{i}= 0 \end{align}​$$

根据这个`KKT`条件的对偶互补条件，当：

1). $\alpha_{i}=0​$

由$C-\alpha_{i}-\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right)=0​$，得到：

$$\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right)= C$$

根据式子$\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right) \xi_{i}= 0$，得到$\xi_i = 0$，最后由原问题的$\left(\boldsymbol{w}^{\mathrm{T}} \boldsymbol{x}_{i}+b\right) \geq 1 - \xi_i $，有：

$$\left(\boldsymbol{w}^{\mathrm{T}} \boldsymbol{x}_{i}+b\right) \geq 1$$

2). $0 < \alpha_i < C$

由$\alpha_{i}\left(y_{i}\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right)-1+\xi_{i}\right)=0 ， \left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right) \xi_{i}= 0​$

可以得到$\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right)\alpha_{i}\left(y_{i}\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right)-1\right)=0​$

由$C-\alpha_{i}-\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right)=0​$，得到：

$$(C - \alpha_i)\alpha_i(y_i\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right) - 1) = 0$$

因为$0 < \alpha_i < C$，所以：

$$y_i\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right) = 1$$

再由$\alpha_{i}\left(y_{i}\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right)-1+\xi_{i}\right)=0$，得到：

$\xi_i = 0$

3). $\alpha_i = C$

由$\alpha_{i}\left(y_{i}\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right)-1+\xi_{i}\right)=0 $，有：

$$y_{i}\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right)-1+\xi_{i}=0 \\ \Rightarrow  y_{i}\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right) = 1 - \xi_{i} $$

因为$\xi_i \geq 0$，所以

$$y_{i}\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right) \leq 1$$

综上有：

$$\begin{align} \alpha_{i}=0 \Rightarrow y_{i}\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right) \geq 1 \\ 0<\alpha_{i}<C \Rightarrow y_{i}\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right)=1 \\ \alpha_{i}=C \Rightarrow y_{i}\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right) \leq 1 \end{align}​$$

其中$y_{i}\left(\boldsymbol{w}^{\mathrm{T}} x_i+b\right)$就是每个样本点的函数间隔。

第一种情况，表明$\alpha_i$为正常分类，在分类间隔边界上；第二种情况，表明$\alpha_i$是支持向量，在边界上；第三种情况，表明$\alpha_i$在两条边界之间。

#### $\alpha_i$的上下界

最优解需要满足`KKT`条件，即上述的`3`中情况都需要满足，如果不能满足`KKT`中的$\alpha_i$，就需要更新这些$\alpha_i$，这是第一个约束条件。

更新$\alpha_i$的同时还会受到第二个约束条件$\sum_{i=1}^{n} \alpha_{i} y_{i}=0$的限制，即假设选择$\alpha_1, \alpha_2$，需要保证更新前后满足以下的等式才能保证和为$0$的约束：

$$\alpha_{1}^{\text{new}} y_{1}+\alpha_{2}^{\text{new}} y_{2}=\alpha_{1}^{\text{old}} y_{1}+\alpha_{2}^{\text{old}} y_{2}=\zeta$$

其中$\zeta$为常数。

求解的方法是先求解$\alpha_{2}^{\text{new}}$，然后利用其表示$\alpha_{1}^{\text{new}}$。

为了求解$\alpha_{2}^{\text{new}}$，假设它的上下界分别为$H, L$，就有：

$$L \leq \alpha_{2}^{\text{new}} \leq H$$

下面就综合$0 \leq \alpha_i \leq C, i = 1, \cdots, n$和$ \alpha_{1}^{\text{new}} y_{1}+\alpha_{2}^{\text{new}} y_{2}=\alpha_{1}^{\text{old}} y_{1}+\alpha_{2}^{\text{old}} y_{2}=\zeta$来求$\alpha_{2}^{\text{new}}$的取值范围。

1). $y_1 \neq y_2$

即两个样本分别属于不同类别时，有：$\alpha_{1}^{\text{old}}-\alpha_{2}^{\text{old}}=\zeta$
...
即两个样本分别属于同一类别时，有：$\alpha_{1}^{\text{old}}+\alpha_{2}^{\text{old}}=\zeta$

所以有：

$$L=\max (0, \zeta-C), H=\min (C, \zeta)$$

综上，可以得到$\alpha_{2}^{\text{new}}$的上下界为：

$$\left\{\begin{array}{ll}{L=\max \left(0, \alpha_{2}^{\text { old }}-\alpha_{1}^{\text { old }}\right), H=\min \left(C, C+\alpha_{2}^{\text { old }}-\alpha_{1}^{\text { old }}\right)}, & { y_{1} \neq y_{2}} \\ {L=\max \left(0, \alpha_{2}^{\text { old }}+\alpha_{1}^{\text { old }}-C\right), H=\min \left(C, \alpha_{2}^{\text { old }}+\alpha_{1}^{\text { old }}\right)}, & { y_{1}=y_{2}}\end{array}\right.$$

#### $\alpha_i$的更新

上述确定了$\alpha_i$的边界，下面就需要更新$\alpha_i$的值，依照上述假设，选择$\alpha_1, \alpha_2$两个乘子，进行推导，目标函数如下：

$$\begin{array}{ll}
{W\left(a_{1}, a_{2}\right)} & {=\sum_{i=1}^{n} \alpha_{i}-\frac{1}{2} \sum_{i=1}^{n} \sum_{j=1}^{n} y_{i} y_{j} x_{i}^{T} x_{j} \alpha_{i} \alpha_{j}} \\ {} & {=\alpha_{1}+\alpha_{2}+\sum_{i=3}^{n} \alpha_{i}-\frac{1}{2} \sum_{i=1}^{n}\left(\sum_{j=1}^{2} y_{i} y_{j} \alpha_{i} \alpha_{j} x_{i}^{T} \boldsymbol{x}_{j}+\sum_{j=3}^{n} y_{i} y_{j} \alpha_{i} \alpha_{j} \boldsymbol{x}_{i}^{T} \boldsymbol{x}_{j}\right)} \\ {} & {=\alpha_{1}+\alpha_{2}+\sum_{i=3}^{n} \alpha_{i}-\frac{1}{2} \sum_{i=1}^{2}\left(\sum_{j=1}^{2} y_{i} y_{j} \alpha_{i} \alpha_{j} x_{i}^{T} \boldsymbol{x}_{j}+\sum_{j=3}^{n} y_{i} y_{j} \alpha_{i} \alpha_{j} \boldsymbol{x}_{i}^{T} \boldsymbol{x}_{j}\right)} \\ {} & {-\frac{1}{2} \sum_{i=3}^{n}\left(\sum_{j=1}^{2} y_{i} y_{j} \alpha_{i} \alpha_{j} x_{i}^{T} x_{j}+\sum_{j=3}^{n} y_{i} y_{j} \alpha_{i} \alpha_{j} x_{i}^{T} x_{j}\right)} \\ {} & {=\alpha_{1}+\alpha_{2}+\sum_{i=3}^{n} \alpha_{i}-\frac{1}{2} \sum_{i=1}^{2} \sum_{j=1}^{2} y_{i} y_{j} \alpha_{i} \alpha_{j} x_{i}^{T} x_{j}-\sum_{i=1}^{2} \sum_{j=3}^{n} y_{i} y_{j} \alpha_{i} \alpha_{j} x_{i}^{T} x_{j}} \\ {} & {-\frac{1}{2} \sum_{i=3}^{n} \sum_{j=3}^{n} y_{i} y_{j} \alpha_{i} \alpha_{j} x_{i}^{T} x_{j}} \\ {} & {=\alpha_{1}+\alpha_{2}-\frac{1}{2} \alpha_{1}^{2} x_{1}^{T} x_{1}-\frac{1}{2} \alpha_{2}^{2} x_{2}^{T} x_{2}-y_{1} y_{2} \alpha_{1} \alpha_{2} x_{1}^{T} x_{2}-y_{1} \alpha_{1} \sum_{j=3}^{n} \alpha_{j} y_{j} x_{1}^{T} x_{j}} \\ {} & {-y_{2} \alpha_{2} \sum_{j=3}^{n} \alpha_{j} y_{j} x_{2}^{T} x_{j}+\sum_{i=3}^{n} \alpha_{i}-\frac{1}{2} \sum_{i=3}^{n} \sum_{j=3}^{n} y_{i} y_{j} \alpha_{i} \alpha_{j} x_{i}^{T} x_{j}} \end{array}$$

令

$$g(x_i)=\sum_{j=1}^{n} \alpha_{j} y_{j} x_{i}^{T} x_{j}+b$$

$$v_{i}=\sum_{j=3}^{n} \alpha_{j} y_{j} x_{i}^{T} x_{j}=g\left(x_{i}\right)-\sum_{j=1}^{2} \alpha_{j} y_{j} x_{i}^{T} x_{j}-b$$

则优化目标函数进一步简化为：

$W\left(a_{1}, a_{2}\right)=\alpha_{1}+\alpha_{2}-\frac{1}{2} \alpha_{1}^{2} x_{1}^{T} x_{1}-\frac{1}{2} \alpha_{2}^{2} x_{2}^{T} x_{2}-y_{1} y_{2} \alpha_{1} \alpha_{2} x_{1}^{T} x_{2}-y_{1} \alpha_{1} v_{1}-y_{2} \alpha_{2} v_{2} + constant $

因为只针对$\alpha_1, \alpha_2$进行求导，而$constant$部分的对$\alpha_1, \alpha_2$来说为常数项。

因为$\alpha_{1} y_{1}+\alpha_{2} y_{2}=\zeta$，两边同时乘以$y_1$，得到：

$$\alpha_1 = \zeta-y_1y_2\alpha_2 \Rightarrow \alpha_1 = \zeta - s\alpha_2$$

记$y_1y_2 = s$。

将上式带入$W\left(a_{2}\right)$，得到：

$$W\left(a_{2}\right)=\zeta-s \alpha_{2}+\alpha_{2}-\frac{1}{2}\left(\zeta-s \alpha_{2}\right)^{2} x_{1}^{T} x_{1}-\frac{1}{2} \alpha_{2}^{2} x_{2}^{T} x_{2}-s\left(\zeta-s \alpha_{2}\right) \alpha_{2} x_{1}^{T} x_{2} \\ -y_{1}\left(\zeta-s \alpha_{2}\right) v_{1}-y_{2} \alpha_{2} v_{2}+ constant $$

对$\alpha_2$求偏导得到：

$$\frac{\partial W\left(a_{2}\right)}{\partial a_{2}}=-\mathrm{s}+1+\zeta \mathrm{s} x_{1}^{T} x_{1}-\alpha_{2} x_{1}^{T} x_{1}-\alpha_{2} x_{2}^{T} x_{2} -\zeta s x_{1}^{T} x_{2}+2 \alpha_{2} x_{1}^{T} x_{2}+y_{2} v_{1}-y_{2} v_{2}=0 \\ \Rightarrow \alpha_2^{\text{new}} = \frac{y_{2}\left(y_{2}-y_{1}+y_{1} \zeta\left(x_{1}^{T} x_{1}-x_{1}^{T} x_{2}\right)+v_{1}-v_{2}\right)}{x_{1}^{T} x_{1}+x_{2}^{T} x_{2}-2 x_{1}^{T} x_{2}} $$

令$E_{i}=g\left(x_{i}\right)-y_{i}$，$\eta=x_{1}^{T} x_{1}+x_{2}^{T} x_{2}-2 x_{1}^{T} x_{2}$，这里的$E_{i}$为误差项，$\eta$为学习速率。

根据$\zeta = \alpha_1^{\text{old}} + s\alpha_2^{\text{old}}$，$v_{i}=g\left(x_{i}\right)-\sum_{j=1}^{2} \alpha_{j} y_{j} x_{i}^{T} x_{j}-b$

得到最终的迭代公式

$$\alpha_{2}^{\text{new}}=\alpha_{2}^{\text{old}}+\frac{y_{2}\left(E_{1}-E_{2}\right)}{\eta}​$$

还需要考虑约束条件$0 \leq \alpha_2 \leq C$，即上述的解未经剪辑。

考虑该取值范围，最终的解析解为：

$$\alpha_{2}^{\text{new, clipped}}=\left\{\begin{array}{ll}{H} & {\alpha_{2}^{\text{new}} > H} \\ {\alpha_{2}}^{\text{new}} & {L \leq \alpha_{2}^{\text{new}} \leq H} \\ {L} & {\alpha_{2}^{\text{new}} < L}\end{array}\right.$$

根据$\alpha_1 = \zeta - s\alpha_2$，所以：

$$\alpha_1^{\text{new}} = \alpha_1^{\text{old}} + y_1y_2(\alpha_2^{\text{old}} - \alpha_2^{\text{new, clipped}})$$

**优化——启发式选择$\alpha_2$**

在大量数据集的情况下，这个迭代公式$\alpha_{2}^{\text{new}}=\alpha_{2}^{\text{old}}+\frac{y_{2}\left(E_{1}-E_{2}\right)}{\eta}$的迭代速度会变慢，原因在于$\alpha_{2}^{\text{new}}$的选择是随机的。这里对其进行启发式的选择，来加快$\alpha_{2}^{\text{new}}$的迭代速度。即需要加大$\frac{E_{1}-E_{2}}{\eta}$这个斜率，故每次都在非边界乘子中选择$|E_i - E_j|$最大的样本。

#### $b$的更新

当$\alpha_1^{\text{new}}$在取值范围$0 \leq \alpha_1^{\text{new}} \leq C$之间时，根据`KKT`条件该点即为支持向量上的点，满足以下：

$$y_1(\boldsymbol{w}^T\boldsymbol{x_1} + b) = 1$$

根据$\boldsymbol{w}=\sum_{i=1}^{n} \alpha_{i} y_{i} \boldsymbol{x}_{i}$，公式左右两边乘以$y_1$，得到：

$$\sum_{i=1}^{n} \alpha_{i} y_{i} x_{i} x_{1}+b=y_{1}$$

当$i = 1, 2$时候，有：

$$b_{1}^{\text{new}}=y_{1}-\sum_{i=3}^{n} \alpha_{i} y_{i} x_{i}^{T} x_{1}-\alpha_{1}^{\text{new}} y_{1} x_{1}^{T} x_{1}-\alpha_{2}^{\text{new}} y_{2} x_{2}^{T} x_{1} \\ \Rightarrow b_{1}^{\text{new}}=b^{\text{old}}-E_{1}-y_{1}\left(\alpha_{1}^{\text{new}}-\alpha_{1}^{\text{old}}\right) \boldsymbol{x}_{1}^{T} \boldsymbol{x}_{1} -y_{2}\left(\alpha_{2}^{\text{new}}-\alpha_{2}^{\text{old}}\right) \boldsymbol{x}_{2}^{T} \boldsymbol{x}_{\mathbf{1}}$$

同理，

$$b_{2}^{\text{new}}=b^{\text{old}}-E_{2}-y_{1}\left(\alpha_{1}^{\text{new}}-\alpha_{1}^{\text{old}}\right) \boldsymbol{x}_{1}^{T} \boldsymbol{x}_{2} -y_{2}\left(\alpha_{2}^{\text{new}}-\alpha_{2}^{\text{old}}\right) \boldsymbol{x}_{2}^{T} \boldsymbol{x}_{2}$$

当$b_1, b_2$都有效的时候，两者相等，即：$b^{\text {new}}=b_{1}^{\text { new }}=b_{2}^{\text { new }}$

当两个乘子都在边界上时，阈值$b$与`KKT`条件一致，不满足时，以两者中点为新阈值：

$$b=\left\{\begin{array}{cc}{b_{1},} & {0<\alpha_{1}^{\text { new }}<C} \\ {b_{2}} & {0<\alpha_{2}^{\text {new}}<C} \\ {\left(b_{1}+b_{2}\right) / 2} & {\text { otherwise }}\end{array}\right.$$

#### `SMO`算法步骤

1. 计算误差

$$E_{i}=g\left(x_{i}\right)-y_{i} = \sum_{j=1}^{n} \alpha_{j} y_{j} x_{i}^{T} x_{j}+b - y_i$$

2. 计算上下界

$$\left\{\begin{array}{ll}{L=\max \left(0, \alpha_{j}^{\text { old }}-\alpha_{i}^{\text { old }}\right), H=\min \left(C, C+\alpha_{j}^{\text { old }}-\alpha_{i}^{\text { old }}\right)}, & { y_{i} \neq y_{j}} \\ {L=\max \left(0, \alpha_{j}^{\text { old }}+\alpha_{i}^{\text { old }}-C\right), H=\min \left(C, \alpha_{j}^{\text { old }}+\alpha_{i}^{\text { old }}\right)}, & { y_{i}=y_{j}}\end{array}\right.$$

3. 计算$\eta$

$$\eta=x_{1}^{T} x_{1}+x_{2}^{T} x_{2}-2 x_{1}^{T} x_{2}$$

4. 更新$\alpha_j$

$$\alpha_{2}^{\text{new}}=\alpha_{2}^{\text{old}}+\frac{y_{2}\left(E_{1}-E_{2}\right)}{\eta}$$

5. 根据取值范围对解$\alpha_j$进行剪辑

$$\alpha_{j}^{\text{new, clipped}}=\left\{\begin{array}{ll}{H} & {\alpha_{j}^{\text{new}} > H} \\ {\alpha_{j}}^{\text{new}} & {L \leq \alpha_{j}^{\text{new}} \leq H} \\ {L} & {\alpha_{j}^{\text{new}} < L}\end{array}\right.$$

6. 更新$\alpha_i$

$$\alpha_i^{\text{new}} = \alpha_i^{\text{old}} + y_iy_j(\alpha_j^{\text{old}} - \alpha_j^{\text{new, clipped}})$$

7. 更新$b_1, b_2$

$$b_{1}^{\text{new}}=b^{\text{old}}-E_{i}-y_{i}\left(\alpha_{i}^{\text{new}}-\alpha_{i}^{\text{old}}\right) \boldsymbol{x}_{i}^{T} \boldsymbol{x}_{i} -y_{j}\left(\alpha_{j}^{\text{new}}-\alpha_{j}^{\text{old}}\right) \boldsymbol{x}_{j}^{T} \boldsymbol{x}_{i}$$

$$b_{2}^{\text{new}}=b^{\text{old}}-E_{j}-y_{i}\left(\alpha_{i}^{\text{new}}-\alpha_{i}^{\text{old}}\right) \boldsymbol{x}_{i}^{T} \boldsymbol{x}_{j} -y_{j}\left(\alpha_{j}^{\text{new}}-\alpha_{j}^{\text{old}}\right) \boldsymbol{x}_{j}^{T} \boldsymbol{x}_{j}$$

8. 更新$b$

$$b=\left\{\begin{array}{cc}{b_{1},} & {0<\alpha_{i}^{\text { new }}<C} \\ {b_{2}} & {0<\alpha_{j}^{\text {new}}<C} \\ {\left(b_{1}+b_{2}\right) / 2} & {\text { otherwise }}\end{array}\right.$$

#### 非线性`SVM`和核函数

`SVM`大多是处理线性可分的情况，对于非线性的情况，处理方式是选择一个核函数，即通过某个非线性映射将输入变量映射到高维特征空间中，使其在高维空间线性可分。

$$f(x)=w^{\mathrm{T}} x+b, w=\sum_{i=1}^{n} \alpha_{i} y_{i} x_{i} \\ \Rightarrow f(x) = \sum_{i=1}^{n} \alpha_{i} y_{i} x_{i}x + b​$$

使用内积表示该公式，即：

$$f(\boldsymbol{x})=\sum_{i=1}^{n} \alpha_{i} y_{i}\left\langle\boldsymbol{x}_{i}, \boldsymbol{x}\right\rangle+ b$$

针对线性不可分的情况，需要使用一个非线性映射，将数据映射到特征空间，在特征空间使用线性分类器，故假设该映射为$\emptyset\left(\boldsymbol{x}\right)$，分类函数变形为：

$$f(\boldsymbol{x})=\sum_{i=1}^{n} \alpha_{i} y_{i}\left\langle\emptyset\left(\boldsymbol{x}_{i}\right), \emptyset(\boldsymbol{x})\right\rangle+ b​$$

这个变形后建立线性分类器的方法称为核函数方法。

而核技巧(`Kernel Trick`)就是直接计算$\left\langle\emptyset\left(\boldsymbol{x}_{i}\right), \emptyset(\boldsymbol{x})\right\rangle$，避免分别计算$\emptyset\left(\boldsymbol{x}_{i}\right), \emptyset(\boldsymbol{x})$的过程，即$\left\langle\emptyset\left(\boldsymbol{x}_{i}\right), \emptyset(\boldsymbol{x})\right\rangle=k(\left\langle\emptyset\left(\boldsymbol{x}_{i}\right), \emptyset(\boldsymbol{x})\right\rangle)$。

通过核技巧的转变，分类函数变为：

$$f(\boldsymbol{x})=\sum_{i=1}^{n} \alpha_{i} y_{i} \kappa\left(\boldsymbol{x}_{i}, \boldsymbol{x}\right)+b$$

对偶问题变为：

$$\begin{array}{ll}{\max _{\boldsymbol{\alpha}}} & {\sum_{i=1}^{n} \alpha_{i}-\frac{1}{2} \alpha_{i} \alpha_{j} y_{i} y_{j} \kappa\left(\boldsymbol{x}_{i}, \boldsymbol{x}_{j}\right)} \\ {\text { s.t.}} & {\alpha_{i} \geq 0, i=1,2, \cdots, n} \\ {} & {\sum_{i=1}^{n} \alpha_{i} y_{i}=0} \end{array}$$
...
|拉普拉斯核|$\kappa\left(\boldsymbol{x}_{i}, \boldsymbol{x}_{j}\right)=\exp \left(-\frac{\left|x_{i}-x_{j}\right|}{\sigma}\right)$|$\sigma>0$|
...
其中$1, -1$分别属于两个类别，使用以下函数加载数据集并绘图展示：
...
data_mat, label_mat = load_dataset('../assets/scripts/824_01.txt')
show_dataset(data_mat, label_mat)
```

样本点分布如下图：

![样本点分布](../assets/images/824_04.png)
...
def _kernel_map(X, A, **params):
    ...
    if not params['sigma']: # 未提供 sigma 默认设置为 1 / 样本数量
        params['sigma'] = 1 / m
        
    if params['kernel'] == 'linear': # 线性核函数
        K = X * A.T
    elif params['kernel'] == 'rbf': # 高斯核函数
        for j in range(m):
            delta_row = X[j, :] - A
            K[j] = delta_row * delta_row.T
        K = np.exp(K/(-1 * params['sigma'] ** 2)) 
    elif params['kernel'] == 'laplace': # 拉普拉斯核函数
        for j in range(m):
            delta_row = X[j, :] - A
            K[j] = delta_row * delta_row.T
            K[j] = np.sqrt(K[j])
        K = np.exp(- K/params['sigma'])     
    elif params['kernel'] == 'poly': # 多项式核函数
        K = X * A.T
        for j in range(m):
            K[j] = K[j] ** params['degree']
    elif params['kernel'] == 'sigmoid': # Sigmoid核函数
        K = X * A.T
        for j in range(m):
...
def _calc_Ek(data_mat, label_mat, alphas, b, k):
    ...
    Args:
        data_mat: 样本点矩阵 numpy.mat
        label_mat: 类别矩阵 numpy.mat
        alphas: alpha 值向量 numpy.mat
        b: b 值 int
        k: 迭代次数 int
...
def _select_j(data_mat, label_mat, alphas, b, error_cache, m, i, Ei):
    ...
    Args:
        ...
        alphas: alpha 值向量 numpy.mat
        b: b 值 int
        error_cache: 根据矩阵行数初始化的误差缓存，第一列为是否有效的标志位，第二列为实际的误差E的值 numpy.mat
        m: alpha参数个数 int
        i: 迭代次数 int
        Ei: 误差值 float
...
![线性样本集分类与支持向量](../assets/images/824_05.png)
...
这里准备了一份非线性的训练数据集和测试数据集，使用`rbf`核函数进行映射，设置其$sigma = 1.3$，惩罚参数为$200$，松弛变量$0.0001$，最大迭代次数为$100$，最终的结果为：迭代次数: $6$，训练集错误率: $9.00%$，测试集错误率: $18.00%$。

可以看出`SVM`具有较强的分类学习效果。

##### [完整的支持向量机实现](../assets/scripts/svm.py)

* `Scikit-Learn`库简单实现`SVM`
